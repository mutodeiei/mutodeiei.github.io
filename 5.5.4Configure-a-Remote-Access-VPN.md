# 5.5.4 Configure_a_Remote_Access_VPN
- คุณทำงานเป็นผู้ดูแลระบบความปลอดภัยด้านไอที (IT Security Administrator) สำหรับเครือข่ายองค์กรขนาดเล็ก บางครั้งคุณและผู้ดูแลระบบคนอื่น ๆ จำเป็นต้องเข้าถึงทรัพยากรภายในองค์กรจากภายนอกสำนักงาน ดังนั้นคุณต้องการตั้งค่า Remote Access VPN โดยใช้ pfSense เพื่อให้สามารถเข้าถึงระบบได้อย่างปลอดภัย
-  ในแลบนี้ งานของคุณคือการใช้ pfSense wizard เพื่อสร้างและตั้งค่า OpenVPN Remote Access Server โดยมีเงื่อนไขดังต่อไปนี้
+ 1.ข้อมูลการเข้าสู่ระบบ pfSense
    + Username: admin
    + Password: P@ssw0rd
+ 2.สร้าง Certificate Authority (CA) ใหม่ โดยใช้ค่าดังนี้
    + Name: CorpNet-CA
    + Country Code: GB
    + State: Cambridgeshire
    + City: Woodwalton
    + Organization: CorpNet
+ 3.สร้าง Server Certificate ใหม่ โดยใช้ค่าดังนี้
    + Name: CorpNet
    + Country Code: GB
    + State: Cambridgeshire
    + City: Woodwalton
+ 4.ตั้งค่า VPN Server โดยใช้ค่าดังนี้
    + Interface: WAN
    + Protocol: UDP บน IPv4 เท่านั้น
    + Description: CorpNet-VPN
    + Tunnel Network IP: 198.28.20.0/24
    + Local Network IP: 198.28.56.18/24
    + Concurrent Connections: 4 
    + DNS Server 1: 198.28.56.1
+ 5.การตั้งค่าเพิ่มเติมที่ต้องทำ
    + สร้าง Firewall Rule
    + สร้าง OpenVPN Rule
    + ตั้งค่า OpenVPN Server ที่สร้างขึ้นให้เป็นแบบ Remote Access (User Auth)
    + สร้างและตั้งค่า ผู้ใช้งาน VPN มาตรฐาน (Standard Remote VPN Users) ตามที่กำหนด

      | Username | Password | Full Name |
      | ---- | ---- | ---- |
      | blindley | L3tM31nNow | Brian Lindley |
      | jphillips | L3tM31nToo | Jacob Phillips |
    
# Start Lab

  + 1.ข้อมูลการเข้าสู่ระบบ pfSense
    + User  :  admin
    + Password : P@ssw0rd
  
  ![Login](5.5.4/login.png)

  + 2.สร้าง Certificate Authority (CA) ใหม่ ไปที่ VPN > OpenVPN > Wizards > เลือก Local User Access > กด Next  ใช้ค่าดังนี้
    + Name: CorpNet-CA
    + Country Code: GB
    + State: Cambridgeshire
    + City: Woodwalton
    + Organization: CorpNet

  ![Certificate Authority (CA)](5.5.4/c1.png)

  + 3.สร้าง Server Certificate ใหม่ โดยใช้ค่าดังนี้ จากนั้น > กด Next
    + Name: CorpNet
    + Country Code: GB
    + State: Cambridgeshire
    + City: Woodwalton

  ![Server Certificate](5.5.4/c2.png)

  + 4.ตั้งค่า VPN Server  เมื่อสร้าง Server Certificate กด Next > ก็จะมาหน้า server setup  ใช้ค่าดังนี้ 
    + Interface: WAN
    + Protocol: UDP บน IPv4 เท่านั้น
    + Description: CorpNet-VPN
    + Tunnel Network IP: 198.28.20.0/24
    + Local Network IP: 198.28.56.18/24
    + Concurrent Connections: 4 
    + DNS Server 1: 198.28.56.1

 ![VPN Server](5.5.4/opn.png)

  + 5.การตั้งค่าเพิ่มเติม> เมื่อสร้าง Server Certificate กด Next > จะมาที่หน้า firewall rule configuration แล้วเลือก 
  + - [x]  firewall rule
    - [x]  OpenVPN rule

  ![Config openVPN](5.5.4/FW.png)

  + 6.ตั้งค่า OpenVPN Server ที่เพิ่งสร้างขึ้นให้เป็นแบบ Remote Access โดยใช้การยืนยันตัวตนด้วยผู้ใช้ (User Authentication)> edit > Server Mode > Remote Access

  ![Remote Access](5.5.4/rem2.png)

  + 7.เพิ่มบัญชีผู้ใช้สำหรับพนักงานหรือผู้ดูแลระบบ ไปที่  System > User Manager >กด add แล้วใส่ค่าดังนี้
  
      | Username | Password | Full Name |
      | ---- | ---- | ---- |
      | blindley | L3tM31nNow | Brian Lindley |
      | jphillips | L3tM31nToo | Jacob Phillips |

  ![Create User](5.5.4/UR1.png)

  + 8.จากนั้น กด Score Lab เพื่อเสร็จสิ้น Lab 5.5.4 Configure_a_Remote_Access VPN

  ![Reprot](5.5.4/FNN.png)
